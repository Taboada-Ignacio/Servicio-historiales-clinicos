...
function ConsultarExportacionesHistoriaClinica({ onVolver }) {
  ...
  return <main className="contenido pagina-consulta-exportaciones">
    ...
    {pantalla === 'detalle' && exportacionSeleccionada && <><section className="panel identidad-paciente-epicrisis"><div><p className="sobrelinea">Paciente seleccionado</p><h2>{seleccionado.apellido}, {seleccionado.nombre}</h2></div></section><section className="panel detalle-exportacion-historia"><div className="cabecera-detalle-exportacion"><div><p className="sobrelinea">Exportación registrada</p><h2>{exportacionSeleccionada.nombreArchivo}</h2><p>{formatearFecha(exportacionSeleccionada.fechaHoraExportacion)}</p></div><span className={`formato-exportacion formato-${(exportacionSeleccionada.formatoArchivoFinal || exportacionSeleccionada.formato).toLowerCase()}`}>{exportacionSeleccionada.formatoArchivoFinal || exportacionSeleccionada.formato}</span></div><dl className="datos-exportacion-historia"><div><dt>Fecha y hora</dt><dd>{formatearFecha(exportacionSeleccionada.fechaHoraExportacion)}</dd></div><div><dt>Tipo de exportación</dt><dd>{etiquetasTipoExportacion[exportacionSeleccionada.tipoExportacion] || 'Historia clínica'}</dd></div><div><dt>Formato de la historia clínica</dt><dd>{exportacionSeleccionada.formatoHistoriaClinica || exportacionSeleccionada.formato}</dd></div><div><dt>Formato del archivo final</dt><dd>{exportacionSeleccionada.formatoArchivoFinal || exportacionSeleccionada.formato}</dd></div><div><dt>Motivo</dt><dd>{etiquetasMotivoExportacion[exportacionSeleccionada.motivo] || exportacionSeleccionada.motivo}</dd></div><div className="dato-exportacion-ancho"><dt>Detalle del motivo</dt><dd>{exportacionSeleccionada.detalleMotivo || 'Sin detalle adicional'}</dd></div><div className="dato-exportacion-ancho"><dt>Nombre del archivo</dt><dd>{exportacionSeleccionada.nombreArchivo}</dd></div></dl></section></>}
  </main>
}
